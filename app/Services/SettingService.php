...
class SettingService
{
    ...
    public function getFromSession(string $key, $default = false)
    {
    ...
    // Gets a setting value from the local cache. Will load the local cache if not previously loaded.
    ...
    // Get the category for the given setting key. Will return 'app' for a general app setting otherwise 'user:<user_id>' for a user setting.
    ...
    protected function loadToLocalCache(string $cacheCategory): void
    {
        ...
        if (!isset($this->localeCache[$cacheCategory])) {
            $this->localeCache[$cacheCategory] = [];
        }
        ...
        foreach ($settings as $setting) {
            ...
            $this->localeCache[$cacheCategory][$setting->setting_key] = $value;
        }
    }

    // Format a settings value.
    protected function formatValue(mixed $value, mixed $default)
    {
    ...
    // Add a setting to the database. Values can be an array or a string.
    ...
    protected function userKey(string $userId, string $key = '')
    {
    ...
    protected function getSettingObjectByKey(string $key)
    {
